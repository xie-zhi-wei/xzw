...
function updateGtid(gtid,$select){
	...
	//第一步：写一个web服务方法
	//第二步：在这里写ajax请求，调用上面的方法
	//第三步：在业务接口扩展一个更新商品类别的方法
	//第四步：在业务层实现类实现相应的方法
	//第五步：在数据层DAO接口拓展一个更新商品类别的方法
	//第六步：在DAO对应的mapper文件中定义相应的update语句
	//第七部：在客户端处理服务端返回的结果，显示气泡
	$.ajax({
...
function queding(a){
	//获取填写的内容
	var $tr = $(a).parents("tr");
	var $input1 = $tr.children("td:eq(2)").children("input");//获取第一个文本框内容
	...
	var $goodsid = $tr.children("td:eq(0)").children("input");
...
function xiugai(a){
    ...
    $(a).hide();//自己隐藏
 //   $(a).next("a").css("display","biock");//保存链接出现
    $(a).next("a").show();
}
...
function editTd($td){
	...
	$input.css("width","80px").css("height","30");
	$td.html($input);
}
